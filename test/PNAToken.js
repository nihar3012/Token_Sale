...
contract('PNAToken', function (accounts) {
    var tokenInstance;
    it('initializes the contract with correct value', function () {
        return PNAToken.deployed().then(function (instance) {
            tokenInstance = instance;
            return tokenInstance.name();
        }).then(function (name) {
            assert.equal(name, 'PNA Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function (symbol) {
            assert.equal(symbol, 'PNA', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function (standard) {
            assert.equal(standard, 'PNA Token v1.0', 'has the correct standard');
        })
    });
    ...
    it('transfers token ownership', function(){
        return PNAToken.deployed().then(function(instance){
            tokenInstance = instance;
            return tokenInstance.transfer.call(accounts[1], 777777777777777);
    ...
    it('handles delegated token transfer', function(){
        return PNAToken.deployed().then(function(instance){
            ...
            return tokenInstance.transferFrom(fromAccount, toAccount, 7777, {from : spendingAccount});
        }).then(assert.fail).catch(function(error){
